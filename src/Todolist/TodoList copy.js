...
export default class TodoList extends Component {
    ...
    constructor(props) {
        ...
        let todosArray = JSON.parse(localStorage.getItem('tos'))
    }
    ...
    addTodo(event) {
        ...
        let newTodoObject = {
            id: this.state.todosArray?.length + 1,
        ...
        this.setState(prevState => {
            return {
                todos: [this.state.todosArray.push(newTodoObject)],
                todoTitle: ''
            }
        })
        localStorage.setItem('tos',  JSON.stringify(this.state.todos)) 
       
    }

    removeTodo(todoId) {
     
        let newTodos = this.state.todosArray?.filter(todo => {
